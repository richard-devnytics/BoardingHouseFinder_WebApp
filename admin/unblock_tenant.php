...
if (isset($_GET['id'])) {
    ...
    $sqlUpdateTenant = "UPDATE tenants SET status = 'unblocked' WHERE tenant_id = ?";
    ...
    if ($stmtUpdate) {
        ...
        if ($stmtUpdate->execute()) {
            $sqlUpdateRoom = " UPDATE holiday_homes SET availability_status= 'available' WHERE tenant_id = ?";
            ...
            if ($stmtEmail) {
                ...
                try {
                    ...
                    // Content
                    $mail->isHTML(true);
                    $mail->Subject = 'Account Unblocked';
                    $mail->Body = 'Dear Owner,<br><br>Your account with Tueogan has been unblocked. You may now login as a Tenant.<br><br>Thank you!<br>From: Tueogan-Admin';

                    $mail->send();
                    echo '<script>alert("Owner account has been unblocked. Confirmation email sent successfully."); window.location.href = "view_tenants.php";</script>';
                    exit();
                } catch (Exception $e) {
